...
class SePayOrderFulfillmentTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        botmod.ORDERS_DB_PATH = self.root / "orders_db.json"
        botmod.INVENTORY_PATH = self.root / "inventory.json"
        bank_checker.ORDERS_DB_PATH = self.root / "orders_db.json"
        sepay_webhook.ORDERS_DB_PATH = self.root / "orders_db.json"
    ...
    def _reserve_sqlite_order(self, order: dict[str, object], product_code: str, credential: str) -> None:
        now = str(order["created_at"])
        product_id = f"{product_code.lower()}-product"
        with closing(sqlite3.connect(self.store_db_path)) as connection:
            with connection:
                connection.execute(
                    ...
                    VALUES (?, ?, ?, 1, 'account', ?, ?)
                    """,
                    (product_id, product_code, str(order["product_name"]), now, now),
                )
                connection.execute(
                    "INSERT INTO inventory_items (id, product_id, secret_value, status, created_at) VALUES (?, ?, ?, 'available', ?)",
                    (f"{product_code.lower()}-item", product_id, credential, now),
                )
    ...
    def test_sepay_fulfills_account_preserving_email_2fa_password_format(self) -> None:
        ...
        order = pending_order(order_id="ORD-CHATGPT-001", product_id="CHATGPT", product_name="CHATGPT", total=99000, amount=99000)
        self._reserve_sqlite_order(order, "GPT-PLUS-1M-PRIVATE", credential)
        ...
        with closing(sqlite3.connect(self.store_db_path)) as connection:
            self.assertEqual(
                connection.execute("SELECT status FROM inventory_items WHERE id = 'gpt-plus-1m-private-item'").fetchone()[0],
                "delivered",
            )
    ...
    def test_sepay_matches_normalized_order_id_without_hyphens(self) -> None:
        ...
        self._reserve_sqlite_order(order, "GROK-SUPER-1M-PRIVATE", credential)
    ...
    def test_sepay_fulfills_year_license_from_common_order(self) -> None:
        ...
        order = pending_order(
            ...
            product_name="GIA HAN 1 NAM - 450000",
